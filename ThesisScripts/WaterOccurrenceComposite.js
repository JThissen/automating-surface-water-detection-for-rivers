...
var start = null;
var stop = null;
var useInverseDistanceWeight = null
var dateOfInterest = null;
...
var aoi = null;
var drawnPolygonList = null;
var minValue = null;
var maxValue = null;
var resampleString = "bilinear";
exports.app = function(start, stop, drawnPolygonList, minValue, maxValue, dateOfInterest, weightPower, useInverseDistanceWeight) 
{
    dateOfInterest = ee.Number(dateOfInterest);
    weightPower = ee.Number(weightPower);
    var resampleString = "bilinear";
    var cutoff = { min: minValue, max: maxValue };
    aoi = drawnPolygonList;
    analysisExtent = ee.FeatureCollection([aoi.buffer(500)]);
    ...
    var results = collections.map(function (collection) {
        ...
        images = images.map(function(item){return item.resample(resampleString);})
        ...
        images = Algorithms.filter(images, collection, aoi, {
            skipEmpty: false,
            maxNoDataPixels: -1, 
            maxCloudPixels: 10000,
            maxSnowPixels: -1
        });
        // rendered image
        var renderedImages = images.sort('system:time_start').map(function (image) {
        ...
        var waterImages = images.map(function (image) {
            ...
            waterMask = waterMask.rename('water').addBands(waterMask.mask().rename('mask'))
            .set('system:index', image.get('system:index')).set('system:time_start', image.get('system:time_start'))
            .set("timeDifference", ee.Number(image.get("system:time_start")).subtract(dateOfInterest).abs())
            .set("InverseWeight", ee.Number(1).divide(ee.Number(image.get("system:time_start")).subtract(dateOfInterest).abs().pow(weightPower)));
            return waterMask;
        });
        // water as a feature
    ...
    var inverseWeightSum = ee.Number(waterImages.aggregate_sum("InverseWeight"));
    
    var waterImagesWeighted = waterImages.map(function(image) 
    {
      var weight = ee.Number(image.get("InverseWeight")).divide(inverseWeightSum);
      return image.multiply(weight).float().set("weight", weight)
      .set("Date", ee.Date(image.get("system:time_start")));
    });
    print("Amount of historical images used:", waterImages.select(["water"]).size());
    
    if(useInverseDistanceWeight)
      var waterOccurrence = waterImagesWeighted.select(['water']).sum().divide(waterImagesWeighted.select(['mask']).sum());
    else
    ...
    waterOccurrence.unmask().unitScale(cutoff.min, cutoff.max);
...
Algorithms.filter = function (images, collection, geometry, options) {
    ...
    if (options.skipEmpty) {
        images = images.map(function (image) {
            var value = ee.Dictionary(image.select(collection.visual.bands[0]).reduceRegion(ee.Reducer.firstNonNull(), geometry, 100, null, null, false, 1e9)).values().get(0);
...
Algorithms.Aster.detectWater = function (image, info, returnImage) {
    ...
    var waterScore = image.resample(resampleString).normalizedDifference(['green', 'nir']);
    ...
    if (returnImage) {
        return water.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('cloud_pixels', ee.Dictionary(cloudMask.reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0))
        //.set('cloud_shadow_pixels', ee.Dictionary(cloudShadows.reduceRegion(ee.Reducer.sum(), analysisExtent, scale)).values().get(0))
        .set('snow_pixels', ee.Dictionary(snowMask.reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('water_threshold', th);
    }
    ...
    return ee.FeatureCollection(waterVector.copyProperties(image)).set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('cloud_pixels', ee.Dictionary(cloudMask.reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('cloud_shadow_pixels', ee.Dictionary(cloudShadows.reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('snow_pixels', ee.Dictionary(snowMask.reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('water_threshold', th);
};
Algorithms.Aster.getCloudPixelCount = function (image, info) {
    ...
    var cloudThreshold = 0.1;
    var cloudMask = clouds.gte(cloudThreshold);
    return ee.Dictionary(cloudMask.reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0);
};
Algorithms.Aster.getSnowPixelCount = function (image, info) {
    ...
    var snowThreshold = 0.4;
    var snowMask = snow.gte(snowThreshold);
    return ee.Dictionary(snowMask.reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0);
};
Algorithms.Aster.getNoDataPixelCount = function (image, info) {
    ...
    return ee.Dictionary(mask.not().reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0);
};
Algorithms.Aster.onLayerAdd = function (image, info) {
    ...
    Map.addLayer(waterScore, { palette: Palettes.water }, 'water score (NDWI)');
    ...
    var th = results.threshold;
    var water = i.gte(ee.Image.constant(th));
    Map.addLayer(water.mask(water), {}, 'water mask (NDWI)');
    var waterVector = water.mask(water).reduceToVectors({ geometry: analysisExtent, scale: errorMargin });
    //waterVector = filterToIntersection(waterVector, analysisExtent)
    waterVector = filterToMaximumAreaFraction(waterVector, analysisExtent);
    Map.addLayer(waterVector, { color: '5050ff' }, 'water mask', true, 0.6);
};
...
Algorithms.AsterT.detectWater = function (image, info, returnImage) {
    image = image.clip(aoi).resample(resampleString);
    ...
    var mode = ee.Number(ee.Dictionary(t.reduceRegion(ee.Reducer.mode(), waterPermanentCenter, errorMargin * 5, null, null, false, 1e9)).get('temp'));
    if (debug) {
        print('Mode:', mode);
    }
...
Algorithms.Landsat.onLayerAdd = function (image, info) {
    ...
    var cloudShadows = projectClouds(az, zen, clouds, cloudThreshold);
    // MNDWI
    var mndwi = image.resample(resampleString).normalizedDifference(['green', 'swir']);
    //waterScore = rescale(waterScore, 'img', [-0.2, 0.35])
    //waterScore = waterScore.mask(waterScore)
    Map.addLayer(mndwi, { palette: Palettes.water }, 'water score (MNDWI)', false);
    // NDWI
    var ndwi = image.resample(resampleString).normalizedDifference(['green', 'nir']);
    //let waterScore = rescale(waterScore, 'img', [-0.2, 0.35])
    //waterScore = waterScore.mask(waterScore)
...
Algorithms.Sentinel1.detectWater = function (image, info, returnImage) {
    ...
    if (returnImage) {
        return water.set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('cloud_pixels', 0).set('cloud_shadow_pixels', 0).set('snow_pixels', 0).set('water_threshold', th);
    }
    waterVector = water.mask(water).reduceToVectors({ geometry: analysisExtent, scale: errorMargin });
    return ee.FeatureCollection(waterVector.copyProperties(image)).set('system:time_start', image.get('system:time_start')).set('nodata_pixels', ee.Dictionary(mask.not().reduceRegion(ee.Reducer.sum(), analysisExtent, scale, null, null, false, 1e9)).values().get(0)).set('cloud_pixels', 0).set('cloud_shadow_pixels', 0).set('snow_pixels', 0).set('water_threshold', th);
};;
